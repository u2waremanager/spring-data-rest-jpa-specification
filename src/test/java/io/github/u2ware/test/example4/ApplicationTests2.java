...
import io.github.u2ware.test.MockMvcSupport;
import io.github.u2ware.test.MockMvcSupport.MvcResultTemplateVariables;
...
public class ApplicationTests2 {
	...
	public void before() throws Exception {
		...
		this.$ = new MockMvcSupport(mvc, springDataRestBasePath);
	}
	
	@Test
	public void contextLoads() throws Exception {

		String mto1Link1 = $.POST("/manyToOneSample1s").C(new ManyToOneSample1("mto1Link1")).is2xx().andReturnLink();		
		String mto1Link2 = $.POST("/manyToOneSample1s").C(new ManyToOneSample1("mto1Link2")).is2xx().andReturnLink();		
		...
		String mto3Link1 = $.POST("/manyToOneSample3s").C(new ManyToOneSample3("mto3Link1")).is2xx().andReturnLink();
		String mto3Link2 = $.POST("/manyToOneSample3s").C(new ManyToOneSample3("mto3Link2")).is2xx().andReturnLink();
		...
		String mto4Link1 = $.POST("/manyToOneSample4s").C(new ManyToOneSample4("mto4Link1")).is2xx().andReturnLink();
		String mto4Link2 = $.POST("/manyToOneSample4s").C(new ManyToOneSample4("mto4Link2")).is2xx().andReturnLink();
		...
		String mto5link1 = $.POST("/manyToOneSample5s").C(new ManyToOneSample5("mto5link1")).is2xx().andReturnLink();
		String mto5Link2 = $.POST("/manyToOneSample5s").C(new ManyToOneSample5("mto5link2")).is2xx().andReturnLink();
		...
		MvcResultTemplateVariables v = (MvcResultTemplateVariables)$.variables();
		logger.info("222: "+v);
		logger.info("2: "+v.get("uri"));
		logger.info("2: "+v.get("uri").get());
		logger.info("2: "+v.get("uri").get().getResponse());
		logger.info("2: "+v.get("uri").get().getResponse().getContentAsString());
		
		
//		MvcResultSupport post = $.GET("uri").is2xx().andReturn();
//		$.get(post.body("_links.sample1.href").toString()).is2xx();
		
		
		
		String uri = "http://a.com/a={uri._links.sample1.href}";
		String a = UriComponentsBuilder.fromUriString(uri).build().expand($.variables()).toUriString();
		logger.info(a);
		
//		logger.info("1: "+post);
//		logger.info("1: "+post.get());
//		logger.info("1: "+post.get().getResponse());
//		logger.info("1: "+post.get().getResponse().getContentAsString());
//		
...
//		MvcResultSupport patch = $.GET("uri").is2xx().andReturn();
